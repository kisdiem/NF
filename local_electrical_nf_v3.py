...
class LocalElectricalFieldV3(nn.Module):
    ...
    def forward(self, x):
        ...
        states=[]; releases=[]; excitations=[]; inhibitions=[]; changes=[]; gates=[]; rs=[]; ets=[]
        for _ in range(self.steps):
            ...
            states.append(v_next); releases.append(release); excitations.append(excitation); inhibitions.append(inhibition)
        ...
        self.last_diagnostics={
            ...
            "inhibition_gate_mean":[(i/(beta*a+1e-8)).mean().item() for i,a in zip(inhibitions,[F.conv2d(r,self.inh_kernel,padding=1) for r in releases])],
