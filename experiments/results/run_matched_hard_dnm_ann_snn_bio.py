"""30-epoch hard-task comparison with ANN/SNN widths matched to Bio-easy."""
...
TASKS="checkerboard,noisy_moons100,noisy_spiral100,hierarchical_xor16,variable_parity16,fsm_sequence"
...
class DNM(nn.Module):
    def __init__(self,d,h,c,b=4):
        super().__init__(); self.proj=nn.Linear(d,h); self.w=nn.Parameter(torch.randn(h,b,h)*.08); self.bias=nn.Parameter(torch.zeros(h,b)); self.out=nn.Linear(h,c); self.b=b
    def forward(self,x):
        h=torch.tanh(self.proj(x.flatten(1))); z=torch.einsum('nk,ibk->nib',h,self.w)+self.bias.unsqueeze(0); return self.out(torch.prod(1+.5*torch.tanh(z),dim=2)-1)

class SNN(nn.Module):
    def __init__(self,d,h,c,steps=5):
        super().__init__(); self.proj=nn.Linear(d,h); self.out=nn.Linear(h,c); self.theta=nn.Parameter(torch.full((h,),.5)); self.steps=steps
    def forward(self,x):
        cur=self.proj(x.flatten(1)); v=torch.zeros_like(cur); out=[]
        for _ in range(self.steps): v=.8*v+cur; out.append(torch.sigmoid(4*(v-self.theta)))
        return self.out(torch.stack(out).mean(0))

def bio(d,c): return BioMLP(d,64,c,branches=4,steps=1,temporal=False,membrane_decay=0.,dendrite='soft_threshold',inhibition=True,adaptive_threshold=True,output_mode='mean')
def matched_width(kind,d,c,target):
    # ANN: (d+1)h+(h+1)c; SNN adds one threshold per hidden unit.
    coeff=d+1+c+(1 if kind=='snn-matched' else 0)
    base=c; return max(1,round((target-base)/coeff))

def make(kind,d,c,target):
    if kind=='bio-easy': return bio(d,c)
...
def run(task,kind,args):
    ...
    sec=time.perf_counter()-start; return {'task':task,'model':kind,'seed':args.seed,'bio_target_parameters':target,'parameters':p,'seconds':sec,'best_test_acc':max(r['test_acc'] for r in hist),'final_test_acc':hist[-1]['test_acc'],'history':hist}

def main():
    p=argparse.ArgumentParser(); p.add_argument('--tasks',default=TASKS); p.add_argument('--models',default='ann-matched,dnm,snn-matched,bio-easy'); p.add_argument('--epochs',type=int,default=30); p.add_argument('--n',type=int,default=6000); p.add_argument('--batch',type=int,default=128); p.add_argument('--lr',type=float,default=3e-3); p.add_argument('--seed',type=int,default=0); p.add_argument('--device',default='cuda' if torch.cuda.is_available() else 'cpu'); p.add_argument('--result',required=True); a=p.parse_args(); rows=[]; os.makedirs(os.path.dirname(os.path.abspath(a.result)),exist_ok=True)
